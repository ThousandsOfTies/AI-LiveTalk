...
/** 感情スロットのVRMAのURLを解決する（blob:はそのまま、それ以外はBASE_URLを付加） */
function _resolveVrmaUrl(path) {
  return path.startsWith('blob:') ? path : import.meta.env.BASE_URL + path;
}

/** デフォルトモーション（普通スロット）をロードしてセレクトを同期する */
async function loadDefaultVRMA(isIdle = false) {
  await viewer.loadVRMA(_resolveVrmaUrl(_vrmaEmotionMap.neutral), { loop: true, isIdle });
  vrmaPresetSelect.value = 'neutral';
}
...
vrmaFileInput.addEventListener('change', async (e) => {
  const file = e.target.files[0];
  if (!file) return;
  const emotion = vrmaPresetSelect.value || 'neutral';
  setStatus(`「${vrmaPresetSelect.options[vrmaPresetSelect.selectedIndex].text}」のモーションを置き換え中...`);
  loadVRMABtn.disabled = true;
  try {
    // blob URLを作成してスロットに保存（File直接渡しはロード後に解放されるためURL化して保持）
    const blobUrl = URL.createObjectURL(file);
    _vrmaEmotionMap[emotion] = blobUrl;
    // neutral スロットなら isIdle=true でアイドルとして登録
    await viewer.loadVRMA(blobUrl, { loop: true, isIdle: emotion === 'neutral' });
    setStatus(`✅ ${vrmaPresetSelect.options[vrmaPresetSelect.selectedIndex].text} のモーションを更新しました`);
  } catch (err) {
...
// ---- VRMAプリセット選択（感情スロット切り替え） ----
vrmaPresetSelect.addEventListener('change', async () => {
  const emotion = vrmaPresetSelect.value;
  const prevEmotion = vrmaPresetSelect.dataset.current ?? 'neutral';
  setStatus('モーション読み込み中...');
  try {
    const url = _resolveVrmaUrl(_vrmaEmotionMap[emotion] || _vrmaEmotionMap.neutral);
    await viewer.loadVRMA(url, { loop: true, isIdle: emotion === 'neutral' });
    vrmaPresetSelect.dataset.current = emotion;
    setStatus('アニメーション再生中');
  } catch (err) {
    setStatus(`VRMAエラー: ${err.message}`);
    vrmaPresetSelect.value = prevEmotion;
    console.error(err);
  }
});
...
// 感情コールバック: 表情変更 + 対応VRMAをワンショット再生
llm.onEmotionDetected = (emotion) => {
  viewer.applyEmotion(emotion);
  setStatus(`感情: ${emotion}`);
  // 感情スロットのVRMAをワンショットで再生（再生終了後にneutralへ戻る）
  const url = _resolveVrmaUrl(_vrmaEmotionMap[emotion] || _vrmaEmotionMap.neutral);
  viewer.loadVRMA(url, { loop: false, isIdle: false }).catch(e => console.warn('感情VRMA再生失敗:', e));
};
