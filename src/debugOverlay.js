/**
 * モバイルデバッグオーバーレイ
 * URL に ?debug を付けてアクセスすると画面右下にログパネルが表示される。
...
function _buildPanel() {
  ...
  Object.assign(hdr.style, {
    padding: '3px 8px', borderBottom: '1px solid #0a0',
    display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexShrink: '0',
  });
  const title = document.createElement('span');
  title.textContent = '🐛 Debug (?debug)';
  ...
  clearBtn.onclick = () => { _lines.length = 0; _render(); };
...
export function initDebugOverlay() {
  ...
  _push('#fff', 'DEBUG', ['パネル起動。メッセージを送ると TTS ログが表示されます。']);
}
